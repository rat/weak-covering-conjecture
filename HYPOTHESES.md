...
| H-003 | Check Bajnok's sumset/critical-number theory for a non-computational bound on j*(l) | open-unexplored | high | Full paper read (arXiv:1512.03038, all 14 pages). Verdict: does not map, and not a close call. R_{l-1,j} mod 3^l is a rank-weighted subset-sum (element value depends on its rank within the chosen subset, confirmed by numerically reproducing the verified l=2,j=2 image {1,2,5,7} mod 9 from this parametrization), while every Bajnok sumset type (hA, restricted-hA, signed-hA) requires each element's contribution to be independent of what else was chosen. Bajnok's critical-number machinery also answers a different question (worst case over all m-subsets) than j*(l) (behavior of one specific structured family). Full derivation in notes/H-003.md. Status kept open-unexplored, not closed, since a bespoke argument using similar techniques (polynomial method, Erdos-Heilbronn-style) directly on R_{j,k}, rather than applying this survey's theorems, remains a real, unexplored possibility; flagged as needing a second opinion before further effort (Rule 11b). | notes/H-003.md | 2026-07-27 | |

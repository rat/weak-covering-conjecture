...
| H-003 | Check Bajnok's sumset/critical-number theory, and a bespoke polynomial-method argument, for a non-computational bound on j*(l) | open-unexplored | high | Two independent negative results now, both with a full derivation in notes/H-003.md. (1) Bajnok's survey (arXiv:1512.03038): does not map, R_{l-1,j} mod 3^l is a rank-weighted subset-sum, not any of Bajnok's sumset types, which all require each element's contribution to be independent of what else was chosen. (2) Direct polynomial-method / Erdos-Heilbronn attempt on R_{j,k} itself (researcher-authorized follow-up): blocked by two independent obstacles - Z/3^l Z has zero divisors for l>=2 (verified computationally: x^2 has 3 roots mod 9, more than its degree, breaking the root-counting lemma Combinatorial Nullstellensatz's proof needs), and separately, R_{j,k}'s rank-dependent weighting isn't expressible as a symmetric function of the chosen set the way the polynomial method's sumset machinery requires, independent of which ring is used. Reduction to F_3 loses the fine mod-3^l structure the problem is about; lifting to the 3-adic integers Z_3 avoids the ring issue but reduces to H-002's already-known territory. One unexplored reframing flagged (source-side polynomial method on the ordered exponent/index domain, mapped to Z/3^l Z only at evaluation) - untested, a real open research question, explicitly flagged as needing a second opinion (different-vendor model or the researcher's own math background) before further investment, not something this session's own confidence should settle, per Rule 11b. | notes/H-003.md | 2026-07-27 | |

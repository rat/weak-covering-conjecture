// Round-5 H-013 budget-sweep variant of h013_holdouts (same verified DP).
//
// Differences from h013_holdouts: adds a mod-9 class histogram, a
// --summary flag (suppresses the full holdout list, needed at intermediate
// budgets where |H| runs to millions), and --dump <path> (raw little-endian
// u64 holdout values, for cross-budget/cross-level analysis in Python).
// Kept as a separate binary so h013_holdouts' output format stays stable.
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("usage: h013_sweep <ell> <j> [--summary] [--dump <path>]");
        std::process::exit(2);
    }
    ...
    if let Some(path) = dump_path {
        let mut bytes = Vec::with_capacity(holdouts.len() * 8);
        for &value in &holdouts {
            bytes.extend_from_slice(&value.to_le_bytes());
        }
        std::fs::write(&path, bytes).expect("failed to write dump");
        println!("dump_written={} count={}", path, holdouts.len());
    }
    println!("class_histogram_mod_3^2={:?}", class_histogram(&holdouts, 9));
    println!(

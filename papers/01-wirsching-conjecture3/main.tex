...
\begin{proof}
Split \eqref{eq:R} at $|y|=\theta_N=N^{-1/3}$. On the central region, Lemma~\ref{lem:taylor} bounds
the cubic remainder $|\Lambda(y)+Vy^2/2|$ by $B\theta_N^3$, which stays bounded (not $o(1)$: this is
exactly why the limit constant below carries a factor $e^{1/3}$) as $N\to\infty$, since $B=O(N)$ and
$\theta_N^3=N^{-1}$; this suffices, via $\int_\R e^{-Vy^2/2}|y|^3\,dy = 4/V^2$ and
Lemma~\ref{lem:variance}'s bound $V=N+O(1)$, to make the resulting error term $O(N^{-1/2})$. On the
tail region, Lemma~\ref{lem:tail}'s bound integrates to a term of order $N^{-1/6}e^{-N^{1/3}/2}$,
which requires $\theta_N \gg N^{-1/2}$, satisfied since $\tfrac13 < \tfrac12$.
...
\alpha = B_0$, $2\beta y - 2\beta - \alpha = B_0 - 1/c$, $\alpha y - \beta y^2 = Q(w_0)$ (in their
notation, $y=w_0$).
\end{proposition}

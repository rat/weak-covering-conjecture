...
infinitely often as $\tau\to\infty$ (Lemma~\ref{lem:saddles}).
\end{theorem}
...
since $w=ct$ and $e^c=a=3$, $-Q(w) = \frac{w^2}{2c}-aw = c\bigl(\tfrac12t^2-at\bigr)$ gives
$a^{\frac12t^2-\alpha t} = e^{-Q(w)}$ once $\alpha=a$ (Proposition~\ref{prop:bkidentity} identifies
...
\]
so $D := H(w_{486746})-H(w_{1011118}) = 4.187449477152\times10^{-4}$ (ball radius $<10^{-70}$, same
$250$-bit precision and $R=30$, $M=4$ truncation as above). Since $w_{486746}$ and
...
(the same cancellation mechanism as the series reversion above, verified independently by direct
numerical solution of $\tau_l=w_0-\log(w_0/c-a)$ at $l$ up to $10^{12}$). So

...
Conditional on the certified bound of Proposition~\ref{prop:Hcert}: the stronger asymptotic
$\varphi(t) \sim \kappa\,\varphi_0(t)$ as $t \to 0^+$, for any single constant $\kappa$ and with no
restriction to a comparison class, is false. The ratio $\varphi(t)/\varphi_0(t)$ oscillates
indefinitely as $t \to 0^+$, with amplitude bounded below by a positive, certified constant: it is
governed by the exactly $c$-periodic function $H$ evaluated at the saddle location $w_0(\tau)$,
$\tau=\log(1/t)$, whose successive-period increment $w_0(\tau+c)-w_0(\tau)$ tends to $c$
...
g(3t-2)\bigr)$. The reflection is in fact vacuous, $\varphi(x)=\varphi(1-x)$ identically: $X$ and
...
Berg and Kr\"uppel's comparison function solves the truncated equation $\lambda\, g'(t) = a\,
g(at)$, $a$ here their dilation parameter (this letter is reused in Section~\ref{sec:periodic} for
this paper's own, unrelated linear coefficient of $Q$; the two never appear in the same formula
except in Section~\ref{sec:bk}, where the collision is flagged again at the point it matters), by
Laplace transform and saddlepoint analysis. In their normalization (their equation~(9.6), with
...
\]
and therefore $\operatorname{osc}(H) \in [D,\,D+M_1h]$ for $D := \max_iH(w_i)-\min_iH(w_i)$; this
step is rigorous regardless of how the grid values themselves are evaluated, since
$\max_iH(w_i)\le\sup H$ holds for any finite subset. Evaluating the $N$ grid values from
Proposition~\ref{prop:Hfourier} truncated at $|m|\le10$ in floating point (Arb ball arithmetic
was used only to re-confirm the two located extremal points below, to 40 digits, not the full
sweep), where the same majorant bounds the discarded modes by $2.71\times10^{-43}$, locates the
maximum at

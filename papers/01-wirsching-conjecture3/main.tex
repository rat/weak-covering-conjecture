...
positive, certified constant: it is governed by the exactly $c$-periodic function $H$ evaluated at
the saddle location $w_0(\tau)$, $\tau=\log(1/t)$, whose successive-period increment
$w_0(\tau+c)-w_0(\tau)$ tends to $c$
(Lemma~\ref{lem:saddles}) even though $w_0(\tau)$ itself drifts away from $\tau$ without bound.
\end{theorem}
...
Theorem~\ref{thm:H}'s own pieces), and that this function is genuinely the periodic factor
...
Proposition~\ref{prop:Hfourier}, and certifies that $H$ is not constant.
...
expression, pins down every constant, and proves Corollary~\ref{cor:bk}. Section~\ref{sec:proofs}
proves both theorems and checks Wirsching's actual requirement directly.
Section~\ref{sec:discussion} states what remains open.
...
for every $j$ and $\sum_{j\ge1}2\cdot3^{-j}=1$. It is $C^\infty$, supported on $[0,1]$, and
...
Berg and Kr\"uppel's comparison function solves the truncated equation $\lambda\, \psi'(t) = a\,
...
as an infinite product, in their proof of that proposition: this is the same object as $Q$, $L$,
and $\Delta$ recombine into via $\exp(H)=e^{-Q}e^Le^{-\Delta}$, so Proposition~\ref{prop:Hcert}'s
primary computation from \eqref{eq:Hcomputable} below is, in substance, an evaluation of this
product. What Proposition~\ref{prop:Hfourier}'s Fourier series supplies instead, and their product
form does not, are the derivative bounds \eqref{eq:Hbounds}, on which the rest of the paper's
saddlepoint estimates depend.
\end{remark}
...
$|\zeta(1-i\omega_m)|\le\log(\omega_m+1)+C_H$. Precisely, with $q := e^{-\pi\alpha_H/2}$, $A_H :=
...
\]
reproduces $H(0)-H(\log\tfrac32) = -0.0003771902809439858148\ldots$ directly (ball radius
$<10^{-70}$), matching the floating-point value above to 19 digits. The majorant
\eqref{eq:Hmajorant} gives $\sum_{m>4}|\hat H(m)| \le 9.41\times10^{-20}$, so the discarded
modes move this by less than $3.8\times10^{-19}$; this route alone certifies the stated
interval for $H(0)-H(\log(3/2))$.
...
\]
so $D := H(w_{486746})-H(w_{1011118}) = 4.187449477152\times10^{-4}$. Since $w_{486746}$ and
...
uniformly over $\rho:=2s/3^N\in[1,3)$ (defined below), which is what makes the two theorems of
...
\begin{remark}
The estimate $|R(s)-1|\le E(N)$ is only informative once $E(N)<1$, which first happens at
$N=19$ (see the proof: $E(18)=1.00170\ldots\ge1$, $E(19)=0.95673\ldots<1$); this is why $N\ge19$
is kept as the theorem's hypothesis, though nothing in the proof strictly requires it. Numerically,
$E$ is also strictly decreasing and satisfies $E(N)\le4.18\,N^{-1/2}$ throughout $19\le N\le5000$
(checked directly in the accompanying repository); neither this fact, nor $E(19)<1$ itself, is
used anywhere in this paper, where only $E(N)\to0$ as $N\to\infty$ is needed.
\end{remark}
...
\eqref{eq:Ebound} gives $E(18) = 1.00170\ldots \ge 1$ and $E(19) = 0.95673\ldots < 1$, confirming
that $N\ge19$ is where the stated bound $E(N)<1$ first becomes non-vacuous.
...
\]
the first of the proposition's three claims. This is an identity in $y$, not merely a value match:
...
equation, so $\operatorname{dist}(w_0(\tau_l), c\Z) \to 0$ uniformly on the class, at rate
...
mechanism with a certified, positive amplitude, not by some correctable technical looseness. Every

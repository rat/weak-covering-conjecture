...
The rest of the argument depends on Conjecture 3. It concerns the behaviour of $\varphi$ as its argument tends
...
a computer-assisted result, a rigorous interval-arithmetic certificate (Proposition~\ref{prop:Hcert}),
not a hand proof.
\end{theorem}
...
$\operatorname{osc}(H) \ge 4.1874494771\times10^{-4}$.
\end{proposition}
...
exactly these two points to $40$ digits in Arb ball arithmetic, from \eqref{eq:Hcomputable} and
from the Fourier series alike,
\[
H(w_{486746}) = -0.6267174403736425786\ldots, \qquad H(w_{1011118}) = -0.6271361853213578093\ldots,
\]
so $D := H(w_{486746})-H(w_{1011118}) = 4.187449477152\times10^{-4}$, rigorously enclosed to radius
$<1.2\times10^{-42}$: the Arb ball rounding at this precision is itself far smaller
($<10^{-70}$ per evaluation), so the stated radius is the combined Fourier truncation error just
above, $2\times5.60\times10^{-43}$, one term for each of the two points. Since $w_{486746}$ and
...
argmax and argmin, since it uses only that the two chosen points lie in the domain. (With
$h:=c/N=1.04772\times10^{-6}$ and $M_1 := 1.1977472315550332\times10^{-3}$ from
\eqref{eq:Hbounds}, a Lipschitz argument over the full grid would give the matching upper bound
$\operatorname{osc}(H)\le D+M_1h\approx D+1.2549\times10^{-9}$; this direction requires the
floating-point search not to have missed a higher maximum or a lower minimum anywhere among the
$N$ grid points, which is not independently certified for the full sweep, so it plays no role here.)
\end{proof}
...
\begin{remark}
The estimate $|R(s)-1|\le E(N)$ becomes informative, $E(N)<1$, exactly at $N=19$ (see the proof:
$E(18)=1.00170\ldots\ge1$, $E(19)=0.95673\ldots<1$), the threshold kept as the theorem's hypothesis.
$E$ is also strictly decreasing and satisfies $E(N)\le4.18\,N^{-1/2}$ throughout $19\le N\le5000$
...
\eqref{eq:Ebound} gives $E(18) = 1.00170\ldots \ge 1$ and $E(19) = 0.95673\ldots < 1$, so the bound
$E(N)<1$ is non-vacuous at $N=19$ (whether some smaller $N$ also gives $E(N)<1$ is immaterial to
what follows and is not checked).
...
happens to fix. That phase sits close to the smaller of the two values of $H$ certified in
Proposition~\ref{prop:Hcert}: $H(0) - H(w_{1011118}) = 5.25\times10^{-6}$, against a certified
oscillation of at least $4.18\times10^{-4}$. (Whether $w_{1011118}$ is where $H$ attains its true
minimum is not certified, per Proposition~\ref{prop:Hcert}'s own caveat on the grid search.) We do
not know whether the near-match with phase $0$ is coincidental.

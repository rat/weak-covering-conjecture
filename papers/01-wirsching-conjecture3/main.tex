...
$\varphi_0$ was informal: they give the periodic correction relating the two as an explicit
...
constant $\kappa$ and with no restriction to a comparison class, is false, by
Proposition~\ref{prop:Hcert}'s unconditionally proved non-constancy of $H$. The ratio
...
and~\ref{sec:envelope} shows $\log\varphi(t)-\log\varphi_0(t) = H(w_0(\tau))+o(1)$, where the saddle
...
translate of Rvachev's atomic function $h_3$ \cite{rvachev1990}, and the dilation-$3$, $\lambda=2/3$
member of
Berg and Kr\"uppel's family of functional equations $\lambda\, \psi'(t) = 3\bigl(\psi(3t) -
\psi(3t-2)\bigr)$ (their generic family member, distinct from this paper's own $g$ introduced
below). The reflection is in fact vacuous, $\varphi(x)=\varphi(1-x)$ identically: $X$ and
...
\]
decays exponentially in $m$ while $\zeta(1-i\omega_m)$
grows only polynomially. Precisely, with $\alpha_H := 2\pi/c$, $q := e^{-\pi\alpha_H/2}$, $A_H :=
c^{-1}\sqrt{3\pi/\alpha_H}$ and $C_H := \tfrac32+\alpha_H^{-1}+\tfrac12\sqrt{1+\alpha_H^{-2}}$, the
bound $|\Gamma(-i\omega_m)|\le\sqrt{3\pi/\omega_m}\,e^{-\pi\omega_m/2}$ (valid for $\omega_m\ge
\alpha_H$, since $\sinh x\ge e^x/3$ there) and the Euler-Maclaurin bound
$|\zeta(1-i\omega_m)|\le\log(\omega_m+1)+C_H$ (valid for $\omega_m\ge\alpha_H$) together give the
majorant
\begin{equation}
\label{eq:Hmajorant}
...
H(m)|$, summing the first six terms exactly and bounding the tail $m>6$ by \eqref{eq:Hmajorant}
...
\]
and consequently $\operatorname{osc}(H) := \sup H - \inf H \ge 3.7719\times 10^{-4}$; a grid search
with a Lipschitz enclosure (below) gives the certified bound
$\operatorname{osc}(H) \in [4.1874494771\times10^{-4},\, 4.1874620262\times10^{-4}]$.
\end{proposition}
...
interval arithmetic. Independently, and rigorously in the sense of interval arithmetic, the Fourier
series of Proposition~\ref{prop:Hfourier} gives the same difference in Arb ball arithmetic: the
$m=0$ term cancels, four modes suffice, and the majorant \eqref{eq:Hmajorant} gives
$\sum_{m>4}|\hat H(m)| \le 9.05\times10^{-20}$; the discarded modes therefore move the difference by
less than $3.7\times10^{-19}$. The two routes agreeing to 18 digits is itself strong evidence the
floating-point route's own error estimate is not being violated by an unmodeled effect.

For the oscillation, let $w_i = ic/N$, $0\le i<N$, with $N = 2^{20}$, so $h := c/N =
1.04772\times10^{-6}$. Every real $w$ lies within $h/2$ of some $w_i$ modulo $c$, so \eqref{eq:Hbounds}
gives $|H(w)-H(w_i)| \le M_1h/2$ with $M_1 := 1.1977472315550332\times10^{-3}$, whence
\[
\max_i H(w_i) \;\le\; \sup H \;\le\; \max_i H(w_i)+\frac{M_1h}{2}, \qquad
\min_i H(w_i)-\frac{M_1h}{2} \;\le\; \inf H \;\le\; \min_i H(w_i),
\]
and therefore $\operatorname{osc}(H) \in [D,\,D+M_1h]$ for $D := \max_iH(w_i)-\min_iH(w_i)$. The
lower bound $D\le\operatorname{osc}(H)$ (the direction Theorem~\ref{thm:unrestricted} actually
uses) follows from $\max_iH(w_i)\le\sup H$ for the true values $H(w_i)$; since the working
precision below keeps floating-point evaluation error many orders below $M_1h\approx1.25\times
10^{-9}$, this does not depend on which arithmetic evaluated the grid. The upper bound
$D+M_1h\ge\operatorname{osc}(H)$, not used elsewhere in this paper, additionally requires the
computed extrema not to be underestimates, which is not independently certified for the full sweep
below (only the two located extremal points are re-confirmed in Arb ball arithmetic). Evaluating
the $N$ grid values from Proposition~\ref{prop:Hfourier} truncated at $|m|\le10$ in floating point,
where the same majorant bounds the discarded modes by $2.71\times10^{-43}$, locates the
maximum at
$i = 486746$ and the minimum at $i = 1011118$; re-evaluating $H$ there to $40$ digits, from
\eqref{eq:Hcomputable} and from the Fourier series alike,
\[
H(w_{486746}) = -0.6267174403736425786\ldots, \qquad H(w_{1011118}) = -0.6271361853213578093\ldots .
\]
So $D = 4.187449477152\times10^{-4}$ and $M_1h = 1.2549\times10^{-9}$, which is the stated enclosure.
\end{proof}
...
\end{align}
$E(19)<1$, and $\sqrt N\,E(N) \to \tfrac43e^{1/3}(2\pi)^{-1/2} = 0.742358\ldots$ as $N\to\infty$
(proved below, from $e_1$ alone: $e_2$ and $e_3$ vanish faster than any power of $N$). Numerically,
$E$ is also strictly decreasing and satisfies $E(N)\le4.18\,N^{-1/2}$ throughout $19\le N\le5000$
(checked directly in the accompanying repository); neither fact, nor $E(19)<1$ itself, is used
below, where only $E(N)\to0$ as $N\to\infty$ is needed. Consequently $\varphi(t) =
...
$(2/\sqrt{2\pi})\int_c^\infty e^{-v^2/2}\,dv$, $c=\theta_N\sqrt V$, and bounding via
$\int_c^\infty e^{-v^2/2}\,dv\le e^{-c^2/2}/c$ at $V=V_{\mathrm{lo}}$, gives $e_2$. On
...
\]
Moreover $w_0(\tau+c)-w_0(\tau) = c+O(1/\tau)$ as $\tau\to\infty$.
\end{lemma}
...
\]
the third of the three coincidences asserted in the proposition above. The other two are the
chain rule:
\[
...
happens to fix. That phase sits close to $H$'s minimum: $H(0) - \min H = 5.25\times10^{-6}$ against
a total oscillation of $4.19\times10^{-4}$. We do not know whether this is coincidental.

...
\psi(3t-2)\bigr)$. The reflection is in fact vacuous, $\varphi(x)=\varphi(1-x)$ identically: $X$ and
...
difference in Arb ball arithmetic, rigorously: the $m=0$ term cancels, four modes suffice, and the
majorant \eqref{eq:Hmajorant} gives $\sum_{m>4}|\hat H(m)| \le 9.41\times10^{-20}$, so the discarded
modes move the difference by less than $3.8\times10^{-19}$; this route alone certifies the stated
...
Series-reverting $Y-\log Y=\tau+r$ to second order: writing $W:=\tau+r$, one iteration of
$Y=W+\log Y$ gives $Y=W+\log W+O(\log^2W/W)$, and $\log W=\log\tau+O(1/\tau)$ then gives
$Y = \tau + \log\tau + r + (\log\tau+r)/\tau + O(\tau^{-2}\log^2\tau)$. Since $w_0=Y+ca$ and

...
appears here; his constant $c$, kept here for a verbatim quotation, is unrelated to
$c:=\log3$ introduced in Section~\ref{sec:periodic}):
...
the $m=0$ term cancels, four modes suffice, and the majorant $|\hat H(m)| \le A_H q^m
m^{-1/2}(\log(\alpha_H m+1)+C_H)$, with $\alpha_H = 2\pi/c$, $q = e^{-\pi\alpha_H/2}$, $A_H =
c^{-1}\sqrt{3\pi/\alpha_H}$ and $C_H = \tfrac32+\alpha_H^{-1}+\tfrac12\sqrt{1+\alpha_H^{-2}}$
(from $|\Gamma(-i\omega_m)|=\sqrt{\pi/(\omega_m\sinh\pi\omega_m)} \le \sqrt{3\pi/\omega_m}\,e^{-\pi\omega_m/2}$
for $\omega_m\ge\alpha_H$, and the Euler-Maclaurin bound $|\zeta(1-i\omega_m)|\le\log(\omega_m+1)+C_H$
for $\omega_m\ge\alpha_H$), gives $\sum_{m>4}|\hat H(m)| \le 9.05\times10^{-20}$; the discarded modes
therefore move the difference by less than $3.7\times10^{-19}$.
...
\eqref{eq:Ebound} gives $E(18) = 1.00170\ldots$ and $E(19) = 0.95673\ldots$, the stated $N_0=19$.
...
Lemma~\ref{lem:saddles}. This is the largest of the five.

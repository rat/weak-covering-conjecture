...
(their $a$, $b$, $\alpha$ are unrelated to this paper's identically-named objects, and are not
used again outside this remark; Section~\ref{sec:bk} below reuses $a$ for their dilation parameter
in a way disambiguated there), specialized to $a=3$, $b=3/2$ (exactly this eigenfunction), gives
...
\]
Take $R=30$, $M=4$. For $s\le3/2$ the first tail is at most $1.106\times10^{-30}$, and for $s\ge1$
...
(checked directly in the accompanying repository); neither fact is used below, where only $E(19)<1$
and $E(N)\to0$ are needed. Consequently $\varphi(t) = \varphi_{\mathrm{sp}}(t)\,(1+o(1))$ as
$t \to 0^+$, with no restriction on the sequence of $t$'s.
\end{theorem}
...
that $N\ge19$ is where the stated bound first takes hold.
